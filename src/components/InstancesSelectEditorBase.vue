...
// onMounted(async () => {
//     console.log('INSTANCESSELECTEDITOR onBeforeMount')
//     nShapePid2.value = true;
//     if (props.modelValue) {
//         console.log("This component alreasdy has data")
...
//         console.log(subValues)
//         console.log(props.triple_uid)
//         if (config.value.use_service && nsHasPID.value) {
//             if (nShapePid2.value) {
//                 fetchingRecordLoader.value = true;
//                 // We should only make the request if both of these conditions are met:
//                 // - config.value.use_service && nsHasPID.value is set to true
//                 // - the range of the slot is an association class, i.e. the propClass.value
//                 //   shape does not have a property shape with path equal to ID_IRI
//                 // It takes time to determine the second condition in the specified way, so we can
//                 // try to take a shortcut by checking if it is a blank node?
//                 fetchFromService(
//                     'get-record',
//                     props.modelValue,
//                     allPrefixes
...
function selectItem(item) {
    if (subValues.value.selectedInstance?.value == item.value) {
        console.log("Same item selected, doing nothing.")
        queryText.value = '';
        menu.value = false;
        return
    }
...
function processTempItem(item) {
    ...
    // If it is, we need to create and link all related triple copies
    // and add them to the graph.
    // Then replace the item.props.relatedQuads with the newly created
    // related quads.
    // Then set item.props.isTemp to false.
    console.log(structuredClone(toRaw(item)));
    if (item.props.isTemp) {
        console.log("is temp item!")
...
function getItemsToList() {
    console.log("Getting items to list now!!!")
    // ---
    ...
    let foundQI = null;
    if (subValues.value.selectedInstance) {
        // props.modelValue shows that the value exists from the formData
        // perspective, while subValues.value.selectedInstance shows that the
        // associated item actually exists and is selected in the list.
    ...
    for (const [i, qd] of combinedQuads.entries()) {
        ...
        if (qd.subject.termType === 'BlankNode') {
            ...
            if (matchIndex !== null) {
                ...
                // connected (as subject) to a new set of quads using the same structure.
                // Then we add those quads to the graph, and use the blank node quad
                // for further item creation.

                // const newRelatedTrips = cloneSubgraphWithNewBlankNode(qd, relatedTrips)
                // // console.log(`--- ADding new quads to graph: ${newRelatedTrips.length}`)
                // // rdfDS.data.graph.addQuads(newRelatedTrips)
                // const newQuad = newRelatedTrips.shift()
                // activeQuad = newQuad;
                // activeRelatedTrips = newRelatedTrips;

                // Update: the above approach adds considerable delay.
                // Mainly coming from adding the quads to the graph, this can be delayed perhaps?
                // Some delay still evident after removing the 'addQuads' part

                // IDEA: create only the blank node quad and add the item, then create and add
                // and link all related triples only once selected.
                // We would need to do the same when the user clicks the edit button, and
                // we'd need to whether the select or edit button has been clicked before
                // keep track of "is_copy" in the item
